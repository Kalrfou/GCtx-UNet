...
class PatchExpand(nn.Module):
    """
    Patch upsampling block to expand H, W by 2 and reduce the number of channels to 3.
    """

    def __init__(self, dim_in, norm_layer=nn.LayerNorm):
        """
        Args:
            dim_in: input feature size dimension.
            norm_layer: normalization layer.
        """

        super().__init__()
        self.upsample = nn.ConvTranspose2d(dim_in, 3, 2, 2, 1, bias=False)
        self.conv_down = UpSize(dim=3, keep_dim=True)
        self.norm1 = norm_layer(dim_in)
        self.norm2 = norm_layer(3)
